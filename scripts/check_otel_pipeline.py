...
# ms-customer: pure Kafka producer — no HTTP spans generated by design.
# ms-ordercheck: opentelemetry-instrument not active (bug #67) — no traces until fixed.
TEMPO_NO_TRACE_SERVICES = {"ms-customer", "ms-ordercheck"}
    ...
    if report.results[-1].passed:
        for svc in KEEPER_SERVICES:
            if svc in TEMPO_NO_TRACE_SERVICES:
                print(f"  ℹ️   tempo:{svc:<30} skipped (pure Kafka producer — no HTTP spans)")
            else:
