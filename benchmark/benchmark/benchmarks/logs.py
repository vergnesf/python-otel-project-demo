"""Logs agent benchmark."""

import logging
import statistics
from benchmark.agents.logs import LogsBenchmark
from benchmark.config import BENCHMARK_MODELS, BENCHMARK_TIMEOUT, NUM_TEST_REQUESTS, AGENT_LOGS_URL
...
async def benchmark_logs_agent() -> dict:
    ...
    print_section_header("Logs Agent Benchmarks")
                ...
                for i, result in enumerate(results, 1):
                    if result.get("success", False):
                        ...
                        if result.get("request"):
                            ...
                            print_query(query)
                            print_expected("Log analysis: summary of errors, affected services, error patterns, severity, insights, and recommendations")
                        # Show response
