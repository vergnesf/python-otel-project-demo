...
async def benchmark_metrics_agent() -> dict:
    ...
    available = await check_service_available(benchmark.config.AGENT_METRICS_URL)
    if not available:
        logger.warning("Metrics agent service not available, skipping")
        return {}

    results_by_model = {}
    benchmark_agent = MetricsBenchmark(benchmark.config.AGENT_METRICS_URL, timeout=benchmark.config.BENCHMARK_TIMEOUT)
