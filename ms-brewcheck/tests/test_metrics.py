"""Tests for business metrics counters in ms-brewcheck.
...
def test_brew_orders_processing_errors_increments_on_error_rate(metric_reader):
    before = _get_counter_value(metric_reader, "brew_orders.processing_errors")
    _process_message(_make_msg(), 1.0)
    after = _get_counter_value(metric_reader, "brew_orders.processing_errors")
    assert after - before == 1


def test_brew_orders_processed_increments_on_success(metric_reader):
    before = _get_counter_value(metric_reader, "brew_orders.processed")
    with patch("brewcheck.brewcheck_consumer.requests.post") as mock_post:
        mock_post.return_value.status_code = 201
        _process_message(_make_msg(), 0.0)
    after = _get_counter_value(metric_reader, "brew_orders.processed")
    assert after - before == 1


def test_brew_orders_processing_errors_increments_on_malformed_message(metric_reader):
    msg = MagicMock()
    msg.headers.return_value = []
