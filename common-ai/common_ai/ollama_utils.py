    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code in [200, 404]:  # 404 is OK if model not loaded
                ...
                print(f"{Colors.YELLOW}⚠️  Ollama API returned status {response.status_code}{Colors.END}")
                return False
                
    except Exception as e:
        print(f"{Colors.YELLOW}⚠️  Could not unload Ollama model via API: {e}{Colors.END}")
        return False
