...
class TracesAnalyzer:
            ...
            "traces": [
                {
                    ...
                    "duration_ms": 312,
                    "status": "error",
                    "services": ["customer", "ordercheck", "order"],
                    "spans": [
                        {"service": "customer", "operation": "send_order", "duration_ms": 85},
                        {"service": "ordercheck", "operation": "validate_order", "duration_ms": 120},
                        {"service": "order", "operation": "POST /orders", "duration_ms": 312, "status": "error"},
                    ],
                },
                {
                    "trace_id": "def456",
                    "duration_ms": 285,
                    "status": "ok",
                    "services": ["customer", "ordercheck", "order", "postgres"],
                    "spans": [
                        {"service": "customer", "operation": "send_order", "duration_ms": 78},
                        {"service": "ordercheck", "operation": "validate_order", "duration_ms": 95},
                        {"service": "order", "operation": "POST /orders", "duration_ms": 267},
                        {"service": "postgres", "operation": "INSERT", "duration_ms": 145},
                    ],
                },
            ],
            "total_count": 150,
    ...
    def _analyze_traces(
        ...
        for trace in traces:
            for span in trace.get("spans", []):
                ...
                if key not in bottlenecks:
                    bottlenecks[key] = {"service": service, "operation": operation, "durations": []}
        ...
        if slow_traces:
            summary_parts.append(
                f"Detected {len(slow_traces)} slow traces (>300ms)."
            )
