"""
FastAPI application for the Traces Agent
"""
...
analyzer: TracesAnalyzer | None = None
...
async def lifespan(app: FastAPI):
    ...
    yield
    # Shutdown
    logger.info("Traces Agent shutting down...")
    if analyzer:
...
app = FastAPI(
    lifespan=lifespan,
    title="Traces Agent API",
    description="Specialized agent for analyzing distributed traces from Tempo via MCP",
    version="0.1.0",
)


class AnalyzeRequest(BaseModel):
    """Request to analyze traces"""
...
async def analyze(request: AnalyzeRequest):
    ...
    try:
        logger.info(f"Traces agent received query: {request.query}")
        result = await analyzer.analyze(
            query=request.query,
            time_range=request.time_range,
            context=request.context,
        )
        logger.info("Traces analysis completed")
        return result
    except Exception as e:
        logger.error(f"Traces analysis failed: {e}")
