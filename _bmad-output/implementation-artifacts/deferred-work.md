...
## Deferred from: code review of story 5-1-ms-dispatch (2026-04-12)

- **Consumer created at module level before KAFKA_BOOTSTRAP_SERVERS guard** [`ms-dispatch/dispatch/dispatch_consumer.py`] — `consumer = Consumer(...)` is instantiated at module level with `localhost:9092` fallback before the `sys.exit(1)` guard in `__main__` is reached. Intentional design trade-off: the guard prevents the consume loop from running but doesn't prevent Consumer creation. Required to keep smoke test imports working without setting env vars. Pre-existing pattern across all consumers.

- **`time.sleep(5)` in poll loop blocks SIGTERM for 5s** [`ms-dispatch/dispatch/dispatch_consumer.py`] — On `UNKNOWN_TOPIC_OR_PART` Kafka error, the consumer sleeps 5s before retrying. During this sleep, a SIGTERM signal is received but `running = False` is not checked until the sleep completes, delaying graceful shutdown by up to 5s. Pre-existing in `ms-brewcheck`, `ms-ingredientcheck`, `ms-quality-control`.

- **No retry or dead-letter queue on 4xx/5xx from ms-beerstock** [`ms-dispatch/dispatch/dispatch_consumer.py`] — On HTTP errors other than 200/400, the message is logged and the Kafka offset is auto-committed, silently dropping the order. No retry backoff or dead-letter topic. Learning-lab trade-off — adding a DLQ would require significant Kafka producer setup.

- **Invalid `brew_style` misclassified as backorder** [`ms-dispatch/dispatch/dispatch_consumer.py`, `ms-beerstock/beerstock/routes/beerstock.py`] — If a Kafka message contains a `brew_style` value not in the BeerStyle enum, `ship_beer_stock` receives a string that doesn't match any stock entry → raises `InsufficientBeerStockError` → HTTP 400 → dispatch classifies it as `"backorder"` instead of `"error"`. Pre-existing BeerStockShip schema limitation (`brew_style: str`, no enum validation at the API boundary).
