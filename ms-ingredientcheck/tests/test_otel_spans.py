"""Tests for OTEL span status and W3C trace context propagation in ms-ingredientcheck."""
...
def _make_msg(headers=None, payload=None):
    ...
    msg.value.return_value = json.dumps(payload or {"ingredient_type": "malt", "quantity": 50}).encode()
    return msg


def test_process_ingredient_delivery_span_ok_on_success(span_exporter):
...
def test_process_ingredient_delivery_links_to_producer_trace(span_exporter):
    """process ingredient-deliveries span has a link to the producer span context via W3C headers."""
    ...
    spans = span_exporter.get_finished_spans()
    process_span = next((s for s in spans if s.name == "process ingredient-deliveries"), None)
